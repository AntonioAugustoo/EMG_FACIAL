...
let demoTimestamp = 0;
let sorrisoState = { fase: 'repouso', valorAtual: 80, valorAlvo: 80, tempoNaFase: 0, duracaoFase: 0, proximoEvento: 2000 };
// ===================================================
...
function gerarDadosDemo() {
    ...
    switch (sorrisoState.fase) {
        case 'repouso':
            sorrisoState.valorAtual = 80 + Math.random() * 20 + Math.sin(demoTimestamp / 100) * 10;
            if (sorrisoState.tempoNaFase >= sorrisoState.proximoEvento) iniciarSorriso();
            break;
        case 'ataque':
            const progressoAtaque = sorrisoState.tempoNaFase / sorrisoState.duracaoFase;
            const easeOutAtaque = 1 - Math.pow(1 - progressoAtaque, 3);
            sorrisoState.valorAtual = lerp(80, sorrisoState.valorAlvo, easeOutAtaque) + (Math.random() - 0.5) * 15;
            if (sorrisoState.tempoNaFase >= sorrisoState.duracaoFase) {
                sorrisoState.fase = 'sustentacao';
                sorrisoState.tempoNaFase = 0;
                sorrisoState.duracaoFase = 1000 + Math.random() * 1000;
            }
            break;
        case 'sustentacao':
            sorrisoState.valorAtual = sorrisoState.valorAlvo + (Math.sin(demoTimestamp / 50) * 15) + (Math.random() - 0.5) * 20;
            if (sorrisoState.tempoNaFase >= sorrisoState.duracaoFase) {
                sorrisoState.fase = 'queda';
                sorrisoState.tempoNaFase = 0;
                sorrisoState.duracaoFase = 500 + Math.random() * 300;
            }
            break;
        case 'queda':
            const progressoQueda = sorrisoState.tempoNaFase / sorrisoState.duracaoFase;
            const easeInQueda = Math.pow(progressoQueda, 2);
            sorrisoState.valorAtual = lerp(sorrisoState.valorAlvo, 80, easeInQueda) + (Math.random() - 0.5) * 15;
            if (sorrisoState.tempoNaFase >= sorrisoState.duracaoFase) {
                sorrisoState.fase = 'repouso';
                sorrisoState.tempoNaFase = 0;
                sorrisoState.proximoEvento = 2000 + Math.random() * 4000;
            }
            break;
    }
   
    return {
        raw: Math.max(0, Math.min(4095, Math.floor(sorrisoState.valorAtual))),
        filtered: Math.floor(Math.max(0, Math.min(4095, Math.floor(sorrisoState.valorAtual))) * (0.88 + Math.random() * 0.07))
    };
}
...
function iniciarSorriso() {
    ...
    sorrisoState.tempoNaFase = 0;
    sorrisoState.duracaoFase = 500 + Math.random() * 200;
    const tipo = Math.random();
    sorrisoState.valorAlvo = tipo < 0.4 ? 180 + Math.random() * 40 : (tipo < 0.8 ? 220 + Math.random() * 60 : 280 + Math.random() * 70);
}

function lerp(inicio, fim, t) { return inicio + (fim - inicio) * t; }
// ================================================================
...
function buscarDadosRealTime() {
    ...
    if (isDemoMode) {
        processarAmostraRealTime(gerarDadosDemo());
...
function processarAmostraRealTime(data) {
    ...
    const LIMITE_DE_PICO = 250; 
   
    const highlightClass = newPoint.filtered > LIMITE_DE_PICO ? 'highlight-pulse' : '';
    ...
    dom.dataTableBody.innerHTML = ultimos.map(d => {
        ...
        const destaqueLog = d.filtered > LIMITE_DE_PICO ? 'log-highlight' : '';
...
function iniciarMonitoramento() {
    ...
    if (isDemoMode) {
        sorrisoState = { fase: 'repouso', valorAtual: 80, valorAlvo: 80, tempoNaFase: 0, duracaoFase: 0, proximoEvento: 1000 + Math.random() * 2000 };
...
function pararESalvar() {
    ...
    if (!isDemoMode) fetch('/stop').catch(e => console.error(e));
